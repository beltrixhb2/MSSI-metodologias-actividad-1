...
\begin{description}
    \item[Equipo Editorial (Dueños del Producto):] Representan a la librería \textit{``Relatos de Papel''}. Son los clientes que definen las necesidades comerciales, responsables de la carga de contenidos, gestión del catálogo y validación de requisitos funcionales. No programan, pero definen \textbf{qué} se debe programar.
    
    \item[Área de Atención al Cliente:] Personal de soporte interno que utilizará la plataforma para gestionar incidencias y devoluciones. Su feedback es esencial para validar los flujos de soporte y formularios de contacto antes del lanzamiento.
\end{description}

\subsection*{B. Stakeholders Técnicos y Especialistas}
\begin{description}
    \item[Expertos en UX y Accesibilidad:] Consultores externos o colaboradores puntuales que participan en la ``Reunión de Validación Multilenguaje y Accesibilidad'' (cada 3 \textit{Sprints}) para verificar traducciones y uso de lectores de pantalla. 

    \item[Equipo Scrum:] Los roles definidos en los puntos anteriores (SM, PO y Dev Team) que ejecutan el proyecto.
\end{description}

\subsection*{C. Usuarios Finales}
\begin{description}
    \item[Lectores (Compradores):] El público objetivo de la plataforma. Su feedback se obtendrá mediante métricas de uso y pruebas de usabilidad organizadas en fases avanzadas.
\end{description}
