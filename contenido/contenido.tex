...
En la metodología Scrum definida para el proyecto transversal \textit{``Relatos de Papel''}, establecemos una estructura organizativa clara basada en los tres roles fundamentales: \textbf{Scrum Master}, \textbf{Product Owner} y \textbf{Equipo de Desarrollo}. Para la ejecución de este proyecto académico, los integrantes del Grupo 90 asumirán roles específicos.
...
Responsable: Zhenghao Alejandro Lin Xu

El \textit{Scrum Master} ejercerá como líder servicial (\textit{servant-leader}) y \textit{coach} del equipo, sin asumir funciones de dirección de proyecto tradicional ni asignación de tareas. Su objetivo principal es la eficiencia del proceso y la salud del equipo.
...
\begin{description}
    ...
    \item[Gestión de Impedimentos:] Dado el carácter distribuido del equipo, su labor crítica será eliminar bloqueos técnicos o burocráticos.
...
Responsable: Esperanza López López

El \textit{Product Owner} es el responsable de maximizar el valor del producto entregado y gestionar el \textit{Product Backlog}. Actúa como la ``voz del cliente'' dentro del equipo y posee la visión editorial y comercial de \textit{``Relatos de Papel''}.
...
\begin{description}
    ...
    \item[Priorización por Valor:] Decide el orden de implementación basándose en el retorno de inversión (ROI) y la criticidad para el lanzamiento (MVP), priorizando, por ejemplo, el catálogo y el carrito de compra sobre funcionalidades secundarias.
...
Responsables: Alba Gaona Arévalo, Beltrán Hidalgo Batalla y Daniel Cano Sevilla
...
Para mitigar el riesgo de un equipo reducido, adoptamos un enfoque de \textbf{competencias en forma de T} (\textit{``T-shaped skills''}). Esto significa que, aunque cada miembro tiene una especialidad profunda (ej. \textit{Backend} o \textit{Frontend}), todos tienen conocimientos transversales suficientes para colaborar en otras áreas (como \textit{Testing} o Despliegue) cuando el \textit{Sprint} lo requiera.

\begin{description}
    \item[Multifuncionalidad:] El equipo cubre autónomamente todas las fases: análisis, diseño, codificación, pruebas y despliegue.
    
    \item[Autoorganización:] Nadie (ni siquiera el \textit{Scrum Master}) indica al equipo cómo convertir el \textit{Product Backlog} en incrementos de funcionalidad; ellos deciden técnicamente la mejor solución.
...
Para garantizar el éxito de \textit{``Relatos de Papel''}, es vital identificar y colaborar activamente con los interesados externos al equipo Scrum. Se ha realizado una distinción clara entre el equipo técnico y los interesados de negocio (cliente), cuya retroalimentación es esencial durante la \textit{Sprint Review}. 

Se han identificado los siguientes grupos clave:

\subsection*{A. Stakeholders de Negocio (El Cliente)}

\begin{itemize}
    \item \textbf{Equipo Editorial (Dueños del Producto):} Representan a la librería \textit{``Relatos de Papel''}. Son los clientes que definen las necesidades comerciales, responsables de la carga de contenidos, gestión del catálogo y validación de requisitos funcionales. No programan, pero definen \textbf{qué} se debe programar.
    
    \item \textbf{Área de Atención al Cliente:} Personal de soporte interno que utilizará la plataforma para gestionar incidencias y devoluciones. Su feedback es esencial para validar los flujos de soporte y formularios de contacto antes del lanzamiento.
\end{itemize}

\subsection*{B. Stakeholders Técnicos y Especialistas}
\begin{itemize}
    \item \textbf{Expertos en UX y Accesibilidad:} Consultores externos o colaboradores puntuales que participan en la ``Reunión de Validación Multilenguaje y Accesibilidad'' (cada 3 \textit{Sprints}) para verificar traducciones y uso de lectores de pantalla. 

    \item \textbf{Equipo Scrum:} Los roles definidos en los puntos anteriores (SM, PO y Dev Team) que ejecutan el proyecto.
\end{itemize}

\subsection*{C. Usuarios Finales}
\begin{itemize}
    \item \textbf{Lectores (Compradores):} El público objetivo de la plataforma. Su feedback se obtendrá mediante métricas de uso y pruebas de usabilidad organizadas en fases avanzadas.
\end{itemize}
...
Se distinguen dos niveles de implicación: el \textbf{Equipo Scrum} (\textit{Product Owner}, \textit{Scrum Master} y Desarrolladores), responsables de la ejecución operativa, y los \textbf{Stakeholders}, quienes actúan como consultores e interesados externos.
...
\chapter{Gestión de Requisitos y Alcance}
...
\begin{center}
    \begin{minipage}{0.9\textwidth}
        \begin{quote}
            \textcolor{blue}{Como} usuario final, \textcolor{blue}{quiero} registrarme en la plataforma \textcolor{blue}{para} poder realizar compras.\\[0.5em]
            \textcolor{blue}{Como} administrador, \textcolor{blue}{quiero} añadir nuevos libros al catálogo \textcolor{blue}{para} que estén disponibles para la venta.
        \end{quote}
    \end{minipage}
\end{center}
\vspace{1em}
...
\begin{figure}[H]
    ...
    \caption{Historia de Usuario definida en el \textit{Product Backlog} del proyecto \textit{``Relatos de Papel''} y que recoge una de las funcionalidades que debe implementar el sistema.}\label{fig:historia-usuario-ejemplo}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Ejemplo de matriz de \textit{Story Points} de referencia.}\label{fig:matriz-story-points}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{contenido/imagenes/figura3.png}
    \caption{Etapas de la técnica de estimación planning poker.}\label{fig:planning-poker}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Ejemplo de epopeya y las Historias de Usuario en las que se descompone.}\label{fig:epopeya}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{contenido/imagenes/figura5.jpg}
    \caption{Mapa de historias de usuario del usuario cliente del proyecto \textit{``Relatos de Papel''} construido con Featmap.}\label{fig:mapa-hu-final}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{contenido/imagenes/figura6.jpg}
    \caption{Mapa de historias de usuario del usuario administrador del proyecto \textit{``Relatos de Papel''} construido con Featmap.}\label{fig:mapa-hu-admin}
\end{figure}
...
\begin{center}
    \includegraphics[width=0.85\textwidth]{contenido/imagenes/criterio-aceptacion.png}
\end{center}
...
\chapter{Ciclo de Vida y Eventos}
...
\begin{figure}[H]
    ...
    \caption{Demostración de Sprint Review.}\label{fig:sprint-review}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Plantilla utilizada durante la Sprint Retrospective.}\label{fig:sprint-retrospective}
\end{figure}
...
\textbf{Innovación y Mejora Continua (\textit{Hack Time}):} Inspirados en la cultura de Spotify\cite{kniberg2012spotify}, se reserva la tarde de los martes de cierre de \textit{Sprint} (después de la \textit{Sprint Retrospective}, de 12:15 a 18:00) como un espacio flexible:
\begin{enumerate}
...
\textbf{\textit{Code Freeze} (Jueves S2):} Se recomienda cerrar el código el jueves para dedicar el viernes y lunes de la semana 2 y 3 a pruebas finales de integración y preparación del entorno de demostración.
...
\textbf{Semana 1} & (Sprint anterior) & (Sprint anterior) & \textbf{INICIO SPRINT} \newline \newline \textit{Sprint Planning} \newline (9:15 - 13:15) & \textit{Daily} \newline (9:15) \newline \newline Desarrollo & \textit{Daily} \newline (9:15) \newline \newline Desarrollo \\ \hline
\textbf{Semana 2} & \textit{Daily} \newline (9:15) \newline \newline Desarrollo & \textit{Daily} \newline (9:15) \newline \newline Desarrollo & \textit{Daily} \newline (9:15) \newline \newline Desarrollo & \textit{Daily} \newline (9:15) \newline \newline \textbf{\textit{Code Freeze}} & \textit{Daily} \newline (9:15) \newline \newline Desarrollo \\ \hline
\textbf{Semana 3} & \textit{Daily} \newline (9:15) \newline \newline QA / Demo & \textbf{CIERRE SPRINT} \newline \newline \textit{Review} \newline \textit{Retro} \newline \textit{Hack Time} & \textbf{INICIO SIGUIENTE} & (Siguiente Sprint) & (Siguiente Sprint) \\ \hline
\end{tabular}
\caption{Visualización del Sprint Tipo}
\end{table}

\chapter{Prácticas de Ingeniería y Calidad}
...
\begin{itemize}
    ...
    \item \textbf{\textit{No Bugs}:} No existen errores críticos o de funcionalidad conocidos asociados a la tarea.
...
\begin{figure}[H]
    ...
    \caption{Flujo completo de tarjetas en Jira.}\label{fig:flujo-colaboracion}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Detección:} Se mueve la Historia de Usuario al listado \texttt{Blocked} en el tablero digital inmediatamente.
...
\chapter{Herramientas y Métricas de Seguimiento}
...
\begin{figure}[H]
    ...
    \caption{Ejemplo de \textit{Burn-down chart}.}\label{fig:burndown-chart}
\end{figure}
...
La \textit{Velocity} se calculará mediante la suma de \textit{Story Points} (SP) de todas las Historias de Usuario completadas.
...
2 & 5, 1, 13 & 19 \\ \hline
\end{tabular}
\caption{Ejemplo de \textit{Velocity}}
\end{table}
...
\textbf{1. Jira}
\begin{itemize}
    \item \textbf{Ventajas:} Diseñada para Scrum, incluye métricas nativas (\textit{Burn-Down}, \textit{Velocity}), ideal para proyectos complejos, gran personalización e integración.
...
\begin{itemize}
    ...
    \item \textbf{Desventajas:} No incluye métricas ágiles nativas (requiere \textit{Power-Ups}), menor automatización, no adecuada para proyectos complejos.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Tablero con diferentes Historias de Usuario}\label{fig:tablero-jira}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Backlog en Jira}\label{fig:backlog-jira}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Ejemplo de Historia de Usuario en Jira}\label{fig:hu-jira}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Story Points} completados vs.\ planificados (\textit{Velocity}).
